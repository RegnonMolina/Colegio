...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Host "Este script precisa ser executado como Administrador. Por favor, feche e execute o PowerShell como Administrador." -ForegroundColor -Type Error
    Start-Sleep 5
    exit
}
...
try {
    ...
    Write-Host "[ERROR] Erro crítico: $msg" -ForegroundColor -Type Error
    exit 1
}
    ...
    if (-not (Test-Path $logDir)) {
        try {
            ...
            Write-Host "ERRO: Não foi possível criar o diretório de log '$logDir'. Mensagem: $($_.Exception.Message)" -ForegroundColor -Type Error
        }
    }
    ...
    try {
        ...
        Write-Host "ERRO: Não foi possível escrever no arquivo de log '$logFilePath'. Mensagem: $($_.Exception.Message)" -ForegroundColor -Type Error
    }
...
function Test-Requi-Type ErrorFunctions {
    param (
        [string[]]$FunctionList
    )
...
function Clear-TemporaryFiles {
    Write-Log "Limpando arquivos temporários..." -Type Warning
...
function Clear-WUCache {
    Write-Log "Limpando cache do Windows Update..." -Type Warning
    Stop-Service wuauserv -Force -ErrorAction SilentlyContinue
    Remove-Item "$env:SystemRoot\Softwa-Type Erroristribution\Download\*" -Recurse -Force -ErrorAction SilentlyContinue
    Start-Service wuauserv
    Write-Log "Cache do Windows Update limpo." -Type Success
}

function Clear-DNS {
    Write-Log "Limpando cache DNS..." -Type Warning
    ipconfig /flushdns | Out-Null
    Write-Log "Cache DNS limpo." -Type Success
}

function Optimize-Volumes {
    Write-Log "Otimizando volumes..." -Type Warning
...
function Clear-WinSxS {
    Write-Log "Limpando WinSxS..." -Type Warning
    try {
        Dism.exe /online /Cleanup-Image /StartComponentCleanup /ResetBase | Out-Null
        Write-Log "WinSxS limpo." -Type Success
    } catch { Write-Log "Erro ao limpar WinSxS: $_" -Type Error }
}

function New-ChkDsk {
    Write-Log "Agendando chkdsk /f /r no próximo reboot..." -Type Warning
    try {
        chkdsk $env:SystemDrive /f /r
        Write-Log "chkdsk agendado (confirme no prompt, se solicitado)." -Type Success
    } catch { Write-Log "Erro ao agendar chkdsk: $_" -Type Error }
}

function Remove-WindowsOld {
    Write-Log "Removendo Windows.old..." -Type Warning
    try {
    ...
    } catch { Write-Log "Erro ao remover Windows.old: $_" -Type Error }
}

function Clear-DeepSystemCleanup {
    Write-Log "Fazendo limpeza profunda (cache de update, logs, drivers antigos)..." -Type Warning
    try {
        Remove-Item "$env:SystemRoot\Softwa-Type Erroristribution\Download\*" -Recurse -Force -ErrorAction SilentlyContinue
    ...
    } catch { Write-Log "Erro na limpeza profunda: $_" -Type Error }
}

function Clear-PrintSpooler {
    Write-Log "Limpando spooler de impressão..." -Type Warning
    try {
        ...
        Write-Log "❌ Erro ao limpar spooler: $_" -Type Error
    }
    Show-SuccessMessage
}

function Clear-Prefetch {
    Write-Log "Limpando Prefetch..." -Type Warning
    try {
        Remove-Item "$env:SystemRoot\Prefetch\*" -Force -Recurse -ErrorAction SilentlyContinue
        Write-Log "Prefetch limpo." -Type Success
    } catch { Write-Log "Erro ao limpar Prefetch: $_" -Type Error }
}

function Clear-ARP {
    Write-Log "Limpando cache ARP..." -Type Warning
    try {
        arp -d *
        Write-Log "Cache ARP limpo." -Type Success
    } 
    catch { Write-Log "Erro ao limpar cache ARP: $_" -Type Error }
}
...
Write-Log "Iniciando Remoção Completa do OneDrive..." -Type Error
    ...
    # === NOVO CÓDIGO AQUI ===
    if ($ScriptConfig.ConfirmationRequi-Type Error) {
...
function Remove-Bloatware {
    try {
        Write-Log "Removendo pacotes provisionados para novos usuários..." -Type Info
        ...
        Get-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue | ForEach-Object {
            if (Test-ShouldRemovePackage -PackageName $_.PackageName) {
                Write-Log "Removendo provisionamento de $($_.PackageName)..." -Type Info
                Remove-AppxProvisionedPackage -Online -PackageName $_.PackageName -ErrorAction SilentlyContinue
            }
        }

        Write-Log "Removendo pacotes do usuário atual..." -Type Info

        Get-AppxPackage -AllUsers -ErrorAction SilentlyContinue | ForEach-Object {
            if (Test-ShouldRemovePackage -PackageName $_.Name) {
                Write-Log "Removendo $($_.Name) para o usuário $($_.User.Name)..." -Type Info
                Remove-AppxPackage -Package $_.PackageFullName -ErrorAction SilentlyContinue
            }
        }
        ...
        Write-Log "Erro durante a remoção de Bloatware: $_" -Type Error
    }
}

function Disable-BloatwareScheduledTasks {
    Write-Log "Desativando tarefas agendadas de bloatware e telemetria..." -Type Warning
    $tasks = @(
    ...
    foreach ($task in $tasks) {
        try {
            ...
            Write-Log "Erro ao desativar ${task}: $_" -Type Error
        }
    }
...
function Stop-BloatwareProcesses {
    Write-Log "Encerrando processos dispensáveis em segundo plano..." -Type Warning
    $processes = @(
        "OneDrive",
        "YourPhone",
        "XboxAppServices",
    ...
    foreach ($proc in $processes) {
        try {
            ...
            Write-Log "Erro ao encerrar ${proc}: $_" -Type Error
        }
    }
...
function Remove-StartAndTaskbarPins {
    Write-Log "Removendo pins do Menu Iniciar e Barra de Tarefas..." -Type Warning
    try {
        ...
        Write-Log "Pins removidos (pode ser necessário reiniciar o Explorer)." -Type Success
    } 
    catch {
        Write-Log "Erro ao remover pins: $_" -Type Error
    }
}

function Remove-ScheduledTasksAggressive {
    Write-Log "Removendo tarefas agendadas de bloatware/telemetria (agressivo)..." -Type Warning
    $tasks = @(
    ...
    foreach ($task in $tasks) {
        try {
            ...
            Write-Log "Tarefa $task desativada e removida." -Type Success
        } 
        catch {
            Write-Log "Erro ao remover/desativar ${task}: $_" -Type Error
        }
    }
