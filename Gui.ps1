...
function Get-GuiActionCatalog {
    ...
    @(
        ...
        [pscustomobject]@{ Cat='Instalação'; Titulo='Atualizar Windows e Drivers';  Func='Update-WindowsAndDrivers'; Desc='Instala o modulo PSWindowsUpdate (se faltar) e roda "Get-WindowsUpdate -AcceptAll -Install" (sem reiniciar sozinho -- reinicio fica por sua conta se algum update pedir). Depois roda "winget upgrade --all", que atualiza TODOS os programas instalados via winget (nao so drivers, apesar do titulo).'; Destr=$false }
